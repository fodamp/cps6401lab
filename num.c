...
void diff_central(double* u, int N, double dx, double* du)
{
    ...
    for (i = 1; i < N - 1; ++i)
    {
        du[i] = (u[i + 1] - u[i - 1]) / (2 * dx);  // Central Difference for middle points
    }
...
void diff_forward(double* u, int N, double dx, double* du)
{
    ...
    for (i = 1; i < N - 1; ++i)
    {
        du[i] = (u[i + 1] - u[i]) / dx;  // Forward Difference for middle points
    }
    du[N - 1] = (u[N - 1] - u[N - 2]) / dx;  // Backward Difference for the last point
}
...
void init(double* u, int N, double dx)
{
    ...
    for (i = 0; i < N + 1; ++i)
    {
        u[i] = sin(i * dx);  // Initialize the array with sin(x)
    }
}
...
int main(int argc, char* argv[])
{
    ...
    double* errdu = (double*)malloc((N + 1) * sizeof(double));
    double* dfdx = (double*)malloc((N + 1) * sizeof(double));
